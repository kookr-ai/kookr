...
- **`kb-scout` subagent for survey turns** `[KB-SCOUT-EVAL-2026-W4]` — When you'd otherwise issue 5+ `kb search` calls yourself on a design or debugging task, invoke the `kb-scout` Haiku subagent instead. Pay Haiku tokens for the ranking work, not Opus tokens. Skip for single-fact lookups, mechanical edits, or when you've already done a substantive `kb search` pass this turn.
