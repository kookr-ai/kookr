...
- **`@xterm/headless` + `addon-serialize` replay buffer.** Heavier (~3 MB RSS per session) and introduces a stateful server-side parser that can be poisoned. A 64 KB byte-stream ring buffer in `SessionBridge` covers the observed pre-alt-screen replay use case.
...
- Smaller surface than tmux — `TerminalBackend` is 5 methods; tmux's `TerminalManager` was 10+ with session-option quirks.
...
- Pre-alt-screen output (e.g., OAuth URLs printed before a TUI enters alt-screen) is not replayed by `dtach -r winch` (spike C6). `SessionBridge`'s ring buffer covers this in Main B.
