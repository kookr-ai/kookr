...
- Probe failure, or restart without a live reconciled probe session, re-parks the same task and stops any partial session only after the exact session is proven absent. A created probe never re-parks merely because launch rejected: direct launch, pending promotion, and crash recovery retain the exact `probing` marker, busy circuit ownership, and active/terminal cleanup ownership when physical stop rejects. Concurrent completion, cancellation, or termination still wins the work outcome, but it may retain that cleanup-only marker until runtime reconciliation or startup proves the session absent; only then is the non-terminal task re-parked or the terminal fence released to one unclaimed half-open probe. A live reconciled probe continues and clears its marker, but confirmed degradation recorded at or after that probe began still keeps the circuit degraded
