...
- Probe failure, or restart without a live reconciled probe session, re-parks the same task and stops any partial session only while the task remains non-terminal; startup keeps an interrupted probe half-open/busy until its exact expected terminal is proven absent, and failed terminal cleanup leaves that fail-closed gate intact for operator retry. A concurrent completion, cancellation, or termination wins, releases the cleanup fence to an unclaimed half-open circuit, clears admission metadata, ignores the stale failure for circuit degradation, and prevents re-parking or launch. A live reconciled probe continues and clears its marker, but confirmed degradation recorded at or after that probe began still keeps the circuit degraded
