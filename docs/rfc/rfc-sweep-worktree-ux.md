...
- Each progress message SHALL carry: `runId`, `scope ∈ {sweep, bulkRemove}`, project identifier, 1-based `index`, `total`, `status ∈ {running, done, failed, skipped}`, and — on completion — the per-unit result. **No intra-project sub-phase enum** (round-1 minimalist).
...
- **PR 1 — progress UI only.** Server broadcasts `workspaceSweepProgress` + broadcasts completion; lock-file progress cursor; client renders `SweepProgress`; graceful old/new skew. Pure additive; no policy change, no new removals, no lock-logic change. Trivially revertible.
