...
2. **Bootstrap fallback test (NO `--plugin-dir`)** — *added in v3 per round-2 R6*: start `claude` directly in `<kookr>` WITHOUT `--plugin-dir`. Same prompt. **Pass** = the agent cites the 18-line routing table from `<kookr>/CLAUDE.md` and gives a non-empty placement recommendation (test verifies the interactive-author bootstrap PR2 is designed to handle).
