...
- a claimed half-open probe is durable (`probing`) with its exact session id before adapter launch. Launch failure re-parks only after that session is proven stopped. A rejected stop in direct launch, promotion, or crash recovery retains the exact marker, busy circuit, and possibly-live session ownership; completion/cancellation/termination may win the work outcome but retains this cleanup-only fence until runtime reconciliation or startup proves absence. Non-terminal ownership then degrades/re-parks, while terminal ownership releases to one unclaimed half-open probe. A live reconciled probe clears its marker unless confirmed degradation recorded at or after that probe began still controls the circuit;
