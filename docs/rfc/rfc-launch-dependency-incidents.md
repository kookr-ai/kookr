...
- a claimed half-open probe is durable (`probing`) before adapter launch; launch failure, or restart without a live reconciled probe session, re-parks the same task only while it remains non-terminal. Startup keeps an interrupted probe half-open/busy until its exact expected terminal is reaped, and a reap error deliberately leaves that gate fail-closed. Completion/cancellation/termination wins over stale asynchronous results by releasing the cleanup fence to unclaimed half-open, clearing admission metadata, and preventing circuit degradation, re-parking, or launch; a live reconciled probe clears its marker unless confirmed degradation recorded at or after that probe began still controls the circuit;
