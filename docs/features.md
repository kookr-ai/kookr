...
| F4.4 | **Task lifecycle management** | Tasks are the unit of work; sessions are attempts attached to a task. A task usually has one live managed session, but may retain multiple historical or recovery sessions. Lifecycle: Open → InProgress → Completed/Cancelled/Terminated. Completing or cancelling a task terminates live sessions. Completing a task opens a cleanup confirmation whose worktree/branch checkbox defaults from Settings and can be overridden for that task. Tasks and session metadata are persisted locally in JSON. |
...
| F4.9 | **Token/cost tracking** | Track token usage (input, output, cache read/write) and estimate cost per agent session by incrementally parsing transcript JSONL files, pricing each usage bucket against its producing model. Unknown or non-exact model rows use the legacy fallback rate and expose fallback pricing quality; authoritative legacy totals remain supported. Displayed in finding cards and detail header. |
