...
| F4.12 | **Dependency-aware launch admission** | Playbooks and API launches may declare required launch dependencies. Confirmed degradation creates one durable pending task without starting a worker; its original prompt, cwd, project, agent/model/effort pins, Ralph wiring, dependencies, and idempotency identity are replayed after recovery. Unknown health remains fail-open only without stronger degraded or half-open evidence. Recovery admits one half-open probe and durably marks it before launch; after restart a reconciled live session clears that task marker, while newer confirmed degradation still controls the circuit and an interrupted/dead probe or provider launch failure re-parks the same task only while it remains non-terminal. Startup holds an interrupted probe half-open/busy until its exact terminal is reaped; cleanup failure stays fail-closed. A concurrent completion, cancellation, or termination releases that cleanup fence to an unclaimed half-open circuit, clears admission metadata, ignores the stale failure for circuit degradation, and prevents re-parking or launch. |
