...
| F5.3 | **Status bar** | Bottom: task count, finding count, the oldest live finding wait when at least one finding is active, keyboard shortcut hints, and — when at least five human-reply samples exist in the last 24 hours — a median time-to-unblock chip. Top bar: queue dots showing triage position, findings/healthy counts, and cost where available. |
