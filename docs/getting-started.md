...
- Claude Code CLI, only if you want Kookr to launch Claude Code agents. Install with `curl -fsSL https://claude.ai/install.sh | bash` (or `npm install -g @anthropic-ai/claude-code`), then authenticate once on the machine that runs Kookr before your first launch: run `claude /login` for interactive sign-in, or export `ANTHROPIC_API_KEY` for headless auth. See [Claude Code Setup](https://code.claude.com/docs/en/setup) for the current install and sign-in options. If Claude Code is not authenticated, the launched agent cannot start work and Claude Code prompts you to log in.
