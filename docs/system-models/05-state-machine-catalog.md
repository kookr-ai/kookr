...
| Launch dependency admission | Process-local `LaunchDependencyAdmission` owns circuit transitions; launch/promotion/recovery paths own task markers | Circuit is in-memory; `Task.launchIntent` and `Task.launchAdmission` are durable. On restart, live reconciled `probing` clears its task marker. Interrupted/dead `probing` stays half-open/busy through exact-terminal cleanup, then restores as degraded `parked` only for non-terminal owners; cleanup failure keeps the fence busy. Timeout before the create callback can leave the exact marker with zero session rows until late link/reap. Terminal transitions clear ordinary admission/retry metadata but retain exact `probing` ownership until reconciliation atomically settles the durable marker and process-local circuit—even when stop already proved absence—so the marker alone does not prove a live/uncertain worker and deletion cannot erase it. Confirmed evidence observed through cleanup settlement keeps the circuit degraded. |
