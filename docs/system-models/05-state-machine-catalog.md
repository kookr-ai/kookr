...
stateDiagram-v2
  ...
  Open --> Pending: Recovery probe fails [task remains non-terminal]
  ...
  Terminated --> Open: Developer reopens task [no probing cleanup owner]
  Terminated --> Cancelled: Developer cancels

  Completed --> Open: Developer reopens task [no probing cleanup owner]
  Cancelled --> Open: Developer reopens task [no probing cleanup owner]
```
...
| Open/Pending/InProgress | recovery_probe_failure [exact session proven stopped; task remains non-terminal] | Pending | Server launch admission | The same task is re-parked with its original replay/idempotency identity only after any exact partial session is stopped. If stop rejects, direct launch, promotion, and crash recovery retain `probing`, the busy circuit, and possibly-live session ownership. A concurrent terminal state wins the work outcome; successful owner-controlled cleanup clears its marker immediately, while unresolved cleanup retains the fence until reconciliation/startup proves the exact session absent. |
...
| InProgress | all_sessions_dead | Terminated | `reconcile()` (all sessions dead; no user ack yet) | Since rfc-task-loss-prevention the auto-path lands in `terminated`, not `completed`. |
| Open | all_sessions_dead | Terminated | `reconcile()` backfill (`open` task with only dead sessions is transitioned via `startTask` then `terminateTask` in one pass) | Covers the edge case where a task reverted to Open but all sessions subsequently died. Updated 2026-04-22 |
...
| Terminated | reopen [no probing cleanup owner] | Open | Developer via GUI | User wants to continue — a new session can be launched. Reopen is refused while an exact dependency-probe cleanup marker remains; reconciliation must settle it first. |
...
| Completed | reopen [no probing cleanup owner] | Open | Developer via GUI, **or** crash-recovery (`src/server/crash-recovery.ts`) auto-reopens a `completed` / `terminated` task when a surviving session is discovered after a Kookr restart | Work wasn't right; needs another attempt. A retained exact dependency-probe cleanup marker blocks reopen until reconciliation settles it. |
| Cancelled | reopen [no probing cleanup owner] | Open | Developer via GUI | Changed mind; task is needed after all. A retained exact dependency-probe cleanup marker blocks reopen until reconciliation settles it. |
...
- **Launch reservation** (added 2026-07-02, issue #700): the `Pending → InProgress` promotion is guarded by a synchronous, token-owned in-memory launch reservation (`TaskStore.beginLaunchWithToken`/`endLaunch`, 10-minute TTL, not persisted — see `docs/reports/issue-700-multi-session-attach-audit.md` §4). Exactly one concurrent promoter wins a pending task; while reserved the task is skipped by `getNextPending` and counts against `MAX_ACTIVE_TASKS` in `getActiveCount`. A persistence-only reservation uses the same exclusion fence but does not count as an active worker, so a slow parked/capacity durability barrier cannot consume fleet capacity. Neither form is a new TaskStatus; both die with the server process.
- **Dependency admission marker** (issue #2841): dependency-degraded/probe-busy `parked` is a durable no-slot wait, `half_open_waiting_for_capacity` is a launchable capacity queue marker, and `probing` records a claimed half-open worker attempt with its exact session id. On restart, a probe with a reconciled live session clears its task marker. An interrupted/dead probe is converted back to degraded parked work only after exact-session absence is proven. Cleanup rejection retains `probing` and busy ownership, including as a temporary cleanup-only marker on a terminal task; runtime reconciliation/startup later re-parks non-terminal ownership or releases terminal ownership. While that cleanup-only marker remains, terminal-to-Open transitions are illegal so replay cannot race the exact-session cleanup owner. Unknown collection evidence is fail-open only without stronger confirmed state and cannot erase a degraded/half-open gate. Dependency-blocked parked tasks are excluded from the ordinary pending TTL and launchable `pendingQueueDepth`; capacity-wait probes remain in both.
...
- **Operator Reopen re-queues (issue: reopen-terminated-task).** The GUI Reopen action is *two* transitions, not one: the `reopenTask` command reopens the task to `Open` and then immediately `pendTask`s it to `Pending` (`src/server/use-cases/task-lifecycle-commands.ts`), so the promotion loop relaunches it with a fresh session. A bare stop at `Open` is not viable for a terminal task the operator reopens: its previous sessions are still attached and all dead, so the next liveness reconcile pass (the "all sessions done → terminal" rule in `src/server/reconciliation.ts`) drives it straight back to `Terminated`; and an `Open` task with no live session is not projected into the dashboard snapshot at all (only `Pending` and terminal task-only rows are synthesized — `src/server/use-cases/snapshot-projection.ts`), so it would be invisible. `Pending` is exempt from that reconcile rule and renders as a `pending-<id>` row. The raw `TaskStore.reopenTask` primitive still performs only `terminal → Open` and is reused unchanged by the recovery paths (crash-recovery, failed-probe re-park), which pend or re-attach a session in the same tick.
...
| Schedule lifecycle | `active` (enabled or held), `archived` (tombstone) | `src/core/schedule.ts`, `src/server/schedule-service.ts` | `archived` (issue #2981) retires a loop without deleting it: the row drops out of `list()` and therefore out of firing, health checks, the status snapshot and ROI attribution, while being retained for audit and un-archive. NOT absorbing for bookkeeping — terminal close-out and boot reconcile iterate `listAll()`, so a run that was in flight when its schedule was archived still records its outcome, silently (no failure alert, no fail-closed auto-pause). Un-archive restores the prior enabled/hold state and rebuilds the rollup from the retained ledger |
...
| Resource-watchdog OOM baseline | `absent_legacy`, `persisted_state`, `runtime_sample` | `src/core/resource-watchdog-state.ts`, `src/server/resource-watchdog-service.ts` | Legacy state establishes a baseline on the first readable sample. Restart restores the saved baseline from `resource-watchdog.state.json`; each successfully read counter value advances it and marks it as observed by the current process. A positive delta triggers one evaluation before advancing, while a lower counter rebaselines without triggering. |
...
| Launch dependency admission | Process-local `LaunchDependencyAdmission` owns circuit transitions; launch/promotion/recovery paths own task markers | Circuit is in-memory; `Task.launchIntent` and `Task.launchAdmission` are durable. On restart, live reconciled `probing` clears its task marker. Interrupted/dead `probing` stays half-open/busy through exact-terminal cleanup, then restores as degraded `parked` only for non-terminal owners; cleanup failure keeps the fence busy. Timeout before the create callback can leave the exact marker with zero session rows until late link/reap. Terminal transitions clear ordinary admission/retry metadata. Successful owner-controlled cleanup also settles the circuit and clears `probing` immediately; unresolved cleanup, creation, or circuit ownership retains the exact marker until reconciliation atomically settles it. The marker alone does not prove a live/uncertain worker and deletion cannot erase it. Confirmed evidence observed through cleanup settlement keeps the circuit degraded. |
...
**Startup reconciliation (ADR-008, updated for ADR-014 on 2026-04-22; tmux removed 2026-04-24):** On startup, session state is reconciled from tasks.json (which includes inline session metadata) + `TerminalBackend` liveness queries. Reconciliation queries only `LocalDtachBackend` (see `src/server/reconciliation.ts`) — V8 removed the tmux backend and `src/server/start.ts` hard-rejects `KOOKR_BACKEND=tmux`. Before that reconcile, `recoverLaunchAbandonedMasters` (`src/server/startup-recovery.ts`) compares prior-process `unadopted` manifest markers with live task-session ids. Exact matches are stamped `adopted` and preserved; unmatched marked masters are boundedly reaped even when their socket is still live (issue #2762). Legacy unmarked entries stay on the generic reconcile / 24h unowned path. Remaining sessions with a live dtach socket are reconnected; sessions without one are marked `terminated` (per rfc-task-loss-prevention). Snooze timers and attention events are ephemeral and rebuilt from reconciled session states.
