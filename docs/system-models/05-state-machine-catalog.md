...
| Pending/InProgress | recovery_probe_failure [exact session proven stopped; task remains non-terminal] | Pending | Server launch admission | The same task is re-parked with its original replay/idempotency identity only after any exact partial session is stopped. If stop rejects, direct launch, promotion, and crash recovery retain `probing`, the busy circuit, and possibly-live session ownership. A concurrent terminal state wins the work outcome but may retain this cleanup-only marker; reconciliation/startup releases it only after the exact session is absent. |
