...
| **Terminal Backend** (`terminal-backend.ts`) | Narrow 5-method abstraction (`createSession` / `attachSession` / `listSessions` / `isAlive` / `killSession`) over the persistence layer. `LocalDtachBackend` is the sole production implementation (ADR-014); `FakeTerminalBackend` is used by tests. Selected in `src/server/start.ts`. Launches the agent binary in interactive mode, monitors process exit, cleans up sessions on shutdown. No circuit-breaker wrapper on the dtach path at present |
...
| **Input Sender** | Delivers developer input to the agent. Bytes are written verbatim to the child PTY's stdin via `SessionHandle.write()` — byte-transparent, no encoding. No queuing needed — input goes directly to the running interactive process. `keystroke.ts` provides encoding helpers for agent-specific control sequences |
...
- Input delivery is immediate via terminal keystrokes (send-keys) — no subprocess spawning, no serialization needed (ADR-007 resolves issue #9).
