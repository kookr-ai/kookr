...
flowchart TD
  ...
  Host[TerminalHostBackend<br/>optional bounded IPC facade]
  ...
  Backend --> Dtach
  Backend -. isolated mode .-> Host
  Host -. child owns .-> Dtach
...
| **GrokBuildAdapter** (`grok-build-adapter.ts`) | Concrete implementation for xAI's official Grok Build CLI (issue #1339). Coordinator over pure helpers: per-session `GROK_HOME` composition (monitoring hooks + seeded auth + linked toolkit plugins), a Grok-specific camelCase hook decoder normalizing into `AgentEvent`, advisory build qualification derived from the reviewed `grok-build-compatibility.v1` manifest, and an allowlisted child environment. Registered by default and included in the frontend picker / round-robin; new launches can be halted via the `KOOKR_GROK_BUILD_DISABLE_NEW_LAUNCHES` kill switch |
| **Terminal Backend** (`terminal-backend.ts`) | Session I/O abstraction over the dtach persistence layer. It owns lifecycle (`createSession` / `listSessions` / `isAlive` / `killSession`), input (`write` / `writeSequence`), output (`captureBytes` / `onData` / optional `captureCurrentFrame` — temporary secondary `dtach -a` for a non-destructive current-screen snapshot used when skipping dense absolute-TUI ring replay), transport errors (`onBackendError`), resize, health stats, safe transport reconnect (`reconnectTransport` — rebuilds only the internal `dtach -a` child, preserving the master/agent pids and subscribers without writing input; #1347), bounded post-restart recovery (`verifyRecoveredSession` — after a Kookr restart, classifies each recovered session as recovered-live/idle/unverified by measuring ring-head progress and self-heals only the attach child for wedged sessions expected to be working, again never touching the master/agent; #1345), and boot-only launch-abandonment handoff (`recoverLaunchAbandonedSessions` — compare prior-process `unadopted` markers with exact live task-session ids, preserve matches, boundedly reap the rest; #2762). `LocalDtachBackend` is the sole production implementation (ADR-014); `FakeTerminalBackend` is used by tests. Selected in `src/server/start.ts`. Launches the agent binary in interactive mode, monitors process exit, cleans up sessions on shutdown. No circuit-breaker wrapper on the dtach path at present |
| **Persistent dtach attach + ring buffer** (inside `local-dtach-backend.ts`) | One persistent attach per session, shared by all viewers. Retains source-positioned bytes for initial display and recovery. Protocol v2 resumes a retained parser only when its epoch, geometry and missing byte range match. Unverified continuity is never repaired in place: the client opens an explicit new view (`acceptGap`). Display-only seeds (absolute-TUI reconstruct, no resume cursor) do that automatically on reconnect instead of blocking on a continuity-unavailable overlay. Reconstructed screens remain approximate and can be unavailable |
| **Terminal host** (`terminal-host.ts`) | Optional server-layer facade implementing the terminal contract through bounded inter-process requests. Its child owns the real `LocalDtachBackend`; it is not a second persistence technology or a second owner of the same sessions |
