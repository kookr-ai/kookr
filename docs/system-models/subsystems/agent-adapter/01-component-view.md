...
| **Persistent dtach attach + ring buffer** (inside `local-dtach-backend.ts`) | One persistent attach per session, shared by all viewers. Retains source-positioned bytes for initial display and recovery. Protocol v2 resumes a retained parser only when its epoch, geometry and missing byte range match. Unverified continuity requires an explicit new view; reconstructed screens are approximate and can be unavailable |
