...
| **hungSuspect TTL reclaim** | Process-lifetime counters (`hungSuspectTtlReclaim.*` on `/api/health`) and residual-alerter episode state **reset to 0** on every restart. Session `lastEventAt` is restored into the watchdog so pre-restart silence age continues (hook + pane clocks seeded from that timestamp — [#2045](https://github.com/kookr-ai/kookr/issues/2045)); stranded open-PR fail-safe still holds. | `reclaimedTotal=0` right after `daemon_uptime_reset` is expected until the next successful reclaim. Read skip-reason fields (`skippedUnderTtl`, `skippedOpenPrFailsafe`, …) before filing “reclaim broken.” See [offline recovery card §3](../reference/offline-recovery-card.md#3-hungsuspect-residual-capacity-waste). |
...
2. **Silence age does not reset** when session metadata carries `lastEventAt`. Startup recovery calls `watchdog.registerAgent(tmux, lastEventAt)` and seeds both the hook and pane clocks from that value so `max(hook, pane, token)` keeps pre-restart silence. Agents that were already past `hungSuspectTtlMinutes` of silence can be reclaimed on the first post-boot liveness tick (subject to open-PR / exempt-anomaly / provider-pause fail-safes).
3. **Fresh agents** (no persisted `lastEventAt`) start their silence clock at registration — they need a full TTL after the new process starts.
...
**Operator policy:** After a planned redeploy, wait at least one `hungSuspectTtlMinutes` window (default 25m) *or* inspect skip-reason dominance before treating residual hungSuspect as a reclaim defect. Prefer `hungSuspectTtlReclaim.skipped*` over occupancy alone.
