...
> **Status:** The OSS-extension skills now ship with the `kookr-toolkit` marketplace plugin (`plugin/skills/pr-contribution-excellence/`, `plugin/reviewer-specialists/`, `plugin/skills/pre-pr-review/`). The PreToolUse / PostToolUse **hooks** stay in this repo at `hooks/` and `scripts/` (not under `plugin/`) because they require explicit user-global hook installation, which also fires them for non-agent `gh pr create` (plain terminal, CI) — the Codex fork now loads plugin hooks via `--plugin-dir`, so plugin delivery is no longer blocked, but user-global install still covers the non-agent contexts. `scripts/install-hooks.sh` installs:
>
...
|------|-----------------|---------|--------|
| `pr-workflow-gate.sh` | `PreToolUse` / `Bash(gh pr create*)` | Blocks `gh pr create` until the `pre-pr-review` skill has written a **producer-token** pre-done marker via `scripts/write-pr-gate-marker.sh` (raw `touch` is rejected — issue #1968). Also runs an **opt-in PR checklist gate** (default OFF — see below) that machine-verifies the PR body's anti-drift checklist against the diff | **In this repo at `hooks/pr-workflow-gate.sh`** |
| `gh-pr-merge-gate.sh` | `PreToolUse` / `Bash(gh pr merge*)` | In Kookr-managed sessions (`KOOKR_TASK_ID` set) with `KOOKR_MERGE_REQUIRE_REVIEW` on (default), blocks bare `gh pr merge` on `kookr-ai/kookr` and steers agents to `pnpm merge` / `scripts/kookr-merge.sh` (issue #1968). Repos whose `CLAUDE.md` / `AGENTS.md` documents independent review as advisory (issue #3027) are allowed through so consuming repos such as Lucy are not stalled | **In this repo at `hooks/gh-pr-merge-gate.sh`** |
