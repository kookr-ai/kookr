...
| `gh-pr-merge-gate.sh` | `PreToolUse` / `Bash(gh pr merge*)` | In Kookr-managed sessions (`KOOKR_TASK_ID` set) with `KOOKR_MERGE_REQUIRE_REVIEW` on (default), blocks bare `gh pr merge` and steers agents to `pnpm merge` / `scripts/kookr-merge.sh` (issue #1968) | **In this repo at `hooks/gh-pr-merge-gate.sh`** |
