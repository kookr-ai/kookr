...
**Health-path background publications:** Diagnostics that require unbounded historical scans publish process-scoped snapshots instead of running from `GET /api/health`. The queue-feeder invent-class rollup refreshes immediately at boot and every 60 seconds through one single-flight `InventPriorityHealthRefresher`; health reads only its last completed counts, `generatedAt`/`ageMs`, and `lastRefreshError`. A failed refresh retains the last successful counts, and shutdown stops the cadence (issue #2912).
