...
Now informed by concrete research: agents run in interactive mode inside managed terminal sessions (tmux). Input via `send-keys`, monitoring via transcript JSONL file tailing + hooks for structured events. Terminal capture (capture-pane) is used only for display. See [ADR-007](adr/007-managed-terminal-sessions.md) (supersedes ADR-004's headless approach).
...
- [x] Validate terminal multiplexer tool choice (tmux PoC: session creation, input delivery, display capture)
- [x] Fork aegiscore patterns (adapt for transcript JSONL monitoring + hooks)
- [x] Create managed terminal session: `tmux new-session -d -s <name> claude` (or equivalent)
- [x] Monitor agent via transcript JSONL file tailing (`~/.claude/projects/…/*.jsonl`) for structured events
- [x] Register hooks for real-time structured event notifications (tool use, errors, permission requests)
- [x] Use `capture-pane` only for display rendering in GUI (not for monitoring logic)
- [x] Parse transcript JSONL entries into normalized `AgentEvent` stream
...
**Demo:** Run `npx kookr` → server starts on `http://127.0.0.1:4800`, open it in a browser, launch an agent via the top-bar launcher, and watch its tmux-backed terminal stream live.
...
- [x] "New Task" dialog: task description (natural-language prompt) + working directory + optional completion criteria → spawns agent in managed terminal session
- [x] "Attach to terminal" action: Attach button in DetailPanel and FindingsPanel (copies `tmux attach` command); interactive xterm.js terminal in browser fully satisfies F4.6
- [x] Task lifecycle: Open → InProgress → Completed/Cancelled (persisted in JSON)
- [x] Stop agent from GUI (kill terminal session)
...
- [x] Input box: type response → deliver via terminal keystrokes (`tmux send-keys`)
...
- [x] AI response suggestions: Claude Haiku generates predicted developer responses for waiting agents ([RFC](rfc-smart-response-assist.md))
- [x] Quick action buttons: extract binary/multiple-choice options from agent messages as clickable buttons ([RFC](rfc-smart-response-assist.md))
- [x] AI task naming: Claude Haiku generates concise task names from prompts ([RFC](rfc-ai-task-naming.md))
- [x] Token/cost tracking: incremental transcript parsing tracks tokens and costs per session
- [x] Build version info: commit hash, branch, timestamp shown in TopBar ([RFC](rfc-build-version-info.md))
...
- Agent discovery via existing tmux sessions (detect agents already running in tmux, adopt into Kookr management)
- Agent discovery via `~/.claude/sessions/` (complement tmux-based discovery)
