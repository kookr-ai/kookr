...
When the Phase-2 umbrella-chain backstop is wired, `GET /api/health` includes an
`umbrellaChains` block with its mode, tick/error counters, staleness threshold,
unstick procedure, and per-chain status/review fields.

The `pipelineStarvation.inventByPriorityClass` health block is a process-scoped
24-hour queue-feeder rollup refreshed at boot and every 60 seconds. Health never
opens or splits the decisions ledger: it reads the last in-memory publication,
including `generatedAt`, `ageMs`, and `lastRefreshError`, while a failed refresh
preserves the last successful counts (issue #2912).
...
`resourceWatchdog.persistence` reports cached state-write health as
`{ status, reservationDurable, consecutiveFailures, lastAttemptAt,
lastSuccessAt, lastFailureAt, lastError }`. `status` is `unknown` before the
process attempts a write, `ok` after the latest successful write, and `error`
after the latest failure. `lastError` is capped at 500 characters. A
`spawn_persist_failed` last decision means the watchdog kept its in-memory
...
otherwise it is `unknown`, unavailable values are `null`, and `sampledAt` is
the retained resource-sampler timestamp (or `null` before the first sample).
Health reads this in-memory sample once and never calls `statfs`. The first
...
on-disk size of the two audit logs that no maintenance-prune sweep reaches —
so remaining growth is visible before it can fill the disk (issue #3158). It
is `{ commandAudit: { activeBytes, archiveCount, archiveBytes },
collaborationAudit: { activeBytes } }`: `commandAudit` covers the
command-journal active `audit.jsonl` plus its rotated `audit.*.jsonl` archives,
and `collaborationAudit` the active `collaboration-audit.jsonl` (size-rotated
at 16 MiB with 2 retained `.N` generations, issue #3252; this gauge is the
bounded active file, not those generations). `activeBytes` is `null` when a
file is absent. The gauge uses `stat` plus one bounded, single-level
...
and at most `resultLimit` latest-evaluation repository rows. The block is an
in-memory informational projection: it is omitted when unwired and never
...
`outcome` (`refilled` / `intentional_idle` / `refill_blocked` / `skipped`), a
stable path-free `reason`, `transitionId`, `evaluatedAt`/`ageMs` freshness,
`wouldLaunchCount`, and at most `resultLimit` launched-leaf rows
(`{ url, taskId }`). The block is an in-memory informational projection: it is
omitted when unwired and never changes top-level health or readiness. It lets a
capacity report separate pause-expected silence (`intentional_idle`) from a
post-resume refill failure (`refill_blocked`).

| Endpoint | Description |
| --- | --- |
| `GET /api/health` | Server status, agent count, build info, launch dependency degradation, capacity ledger, a stale-while-revalidate 24h `lessonYield` snapshot when `kookrDir` is set (issues #1538, #1553), and the first-class `ciBlindDebt` / `ci_blind_debt` metric (blind-merge count + retro-verify queue depth; issue #1703). The assembled JSON body is cached for 1s (`HEALTH_BODY_CACHE_MS`) and shared with any in-flight rebuild so concurrent probes do not restampede `viewTasks()` — gauges can be up to 1s stale (issue #2429). `/api/ready` is not cached. The debt block is a cheap JSONL read of the retro-verify spool — soft-omitted if the spool is unreadable. Also a `sessionReaper` block (issues #1720 / #2081): `enabled`, `lastSweepAt`/`lastStaleAttacherSweepAt`, `lastOrphanCount`/`lastTerminalLeakCount`, cumulative `totalSessionsReaped`/`totalStaleAttachersReaped`, a cumulative `killSessionFailedTotal` with `lastKillFailureAt`/`lastKillFailureSessionId` (issue #3155: a session that resists `killSession` is re-selected and re-fails every sweep — this surfaces that leaked-pty precursor instead of it hiding behind a flat `totalSessionsReaped`), plus `effectiveOrphanAgeMs` (null until first sweep) and `underPressure` reflecting the last sweep's pressure-adaptive orphan age — a cheap in-memory read of the last boot/periodic sweep's own counters, never a fresh scan on this request path (issue #1553 lesson). `terminalBackend` also reports `launchAbandonedRecoveredCount` / `launchAbandonedRecoveryFailureCount` (issue #2762): prior-process launch-abandoned dtach masters reaped — or failed to reap — during this boot's exact manifest/task handoff. Informational; a recovered count does not degrade `terminalBackend.status`. Distinct from `sessionReaper` and `hostStaleDtachReaper`. Also a `hostStaleDtachReaper` block (issues #2356, #2384, #3314) when wired: `enabled`, `dryRun`, `softBound`, `maxReapsPerSweep`, `lastSweepAt`, `lastDtachCount`, `lastUnderPressure`, `lastHostStaleDtachReaped`, `totalHostStaleDtachReaped`, `lastReapedAlways`, `lastReapedUnderPressure`, last-sweep skip counters (`skippedLiveAttached`, `skippedUnderBound`, `skippedRateLimited`, `skippedSocketPresent`, `lastEligibleCount`), and a cumulative `killFailedTotal` with `lastKillFailureAt`/`lastKillFailurePid` (issue #3314: a missing-socket-aged master that resists SIGKILL is re-selected and re-fails every sweep — this surfaces that miss instead of it hiding behind a flat `totalHostStaleDtachReaped`; never a pid list) — same cheap in-memory convention; never a `/proc` scan on the health path. When wired, `finishedAwaitingAckTtlReclaim` / `hungSuspectTtlReclaim` project process-lifetime reclaim + skip-reason counters (issues #1884/#2070/#2084, #1935/#2045) as cheap in-memory reads — never a fresh reclaim scan on this path. `hungSuspectTtlReclaim` also carries a bounded sweep-failure signal (`sweepFailuresTotal`, plus a sanitized `lastFailureCategory` — the error class name, never raw text — and `lastFailureAtMs`, both null after a later successful pass; issue #2897) so a TTL-reaper fault that a local liveness-tick boundary now contains — leaving the later provider-pause, cleanup, and promotion legs to still run — is observable, and a failed pass is never mistaken for a successful zero-reclaim; the same signal is exported as the `kookr_hung_suspect_ttl_sweep_failures_total` counter and `kookr_hung_suspect_ttl_sweep_last_failure_timestamp_seconds` gauge. The `capacity` ledger's `finishedAwaitingAckByCause` sub-tally classifies WHY each finishedAwaitingAck task's ack lags — `awaiting_poll` (normal poll/sweep cadence), `ack_sweep_backlog` (auto-close sweep behind), `manual_review_gate` (ask-first, by design), `auto_close_disabled` (config gap) — so the chronic FAA churn can be attacked at its dominant cause rather than with another downstream mitigation (issue #2142); counts sum to `byClass.finishedAwaitingAck` and are also exported as the `kookr_capacity_finished_awaiting_ack_by_cause` Prometheus gauge. The `capacity` ledger also carries `utilizationPct` (nominal `active / maxActiveTasks`) alongside `effectiveUtilizationPct` (productive `effectiveWorking / maxActiveTasks`), and a sibling `capacityThroughputVerdict` block (`{ verdict: "healthy_throughput" | "idle_capacity", effectiveUtilizationPct, utilizationPct, effectiveWorking, phantomActive, active, maxActiveTasks, idleEffectiveSlots }`) keys the verdict on EFFECTIVE occupancy so a nominally-full fleet held open by phantom (`hungSuspect` + `finishedAwaitingAck`) slots reads `idle_capacity`, not "93.8% healthy" (issue #2169). When the hourly prod smoke tick is enabled (issue #1593 / #2031), a `prodSmokeTick` block projects `status` (`ok`/`alert`/`unknown`), `consecutiveFailures`, `failingChecks`, and optional `generatedAt`/`firstFailedAt` from the durable `prod-smoke-tick-alert.json` artifact only — never re-runs smoke checks on this path; the block is omitted when the tick is disabled. On Linux it also exposes a `staleProcesses` gauge (relay-server + dtach process counts and RSS; issue #1723), served stale-while-revalidate off a background `/proc` scan so the health hot path never blocks, plus a first-class `relayOrphanFinding` block (`{ code: "relay_orphan_accumulation", count, bound, rssBytes }`) when the relay-server orphan count exceeds `KOOKR_RELAY_ORPHAN_ALERT_BOUND` (default 5; issue #1885) so sentinel/reflection can cite a stable code rather than a raw count. The `resourceWatchdog` block (issue #1724) includes `pressureWhileDisabled` + `pressureWhileDisabledReason` (issue #2039): true when the actuator is off and cached `staleProcesses.dtach.count` meets the soft bound (default 20) — visibility only, never auto-enables. Also a `hookReplayCheckpoints` block (`{ sessionCount, fileBytes }` or `null` when disabled; issue #2281): in-memory session count from the hook-watcher checkpoint envelope plus `stat().size` for on-disk bytes — never a full JSON parse of the multi-MB checkpoint store on this path. Also a `startupRecovery` block (issues #2351, #3316): compact deferred crash-recovery counts `{ relaunched, skipped, failed, crashLoopSkips, unverified, repaired, generatedAt }` once recovery has completed (summary present or readiness `readyAt`); omitted before then. Counts only — full relaunched/skipped/failed entry lists and post-restart finding lists stay on `GET /api/startup-summary`. `unverified` / `repaired` are post-restart transport-verification counts (zeros when that phase had nothing to report). Cheap in-memory projection of the retained recovery result; never re-runs recovery on this path. Also a `boot` block (issue #2790): the previous process's exit classification `{ status, reason, previousStartedAt, previousShutdownAt, previousSignal, dirtyStreak }` where `status` is `clean` (the prior process wrote a graceful-shutdown marker), `dirty` (the prior marker was still `running` — a crash/OOM/SIGKILL), or `unknown` (first boot, or a wiped/corrupt marker), and `reason` is `clean_shutdown` / `unclean_exit` / `no_prior_marker` / `marker_unreadable`. `dirtyStreak` (issue #3077) is the count of consecutive dirty boots in an unbroken run: it increments on each dirty boot, resets to `0` on a clean boot, and self-heals to `0` when the marker is missing/corrupt — so a value climbing past `1` marks a whole-process crash loop, not a single restart after a blip. Classified once at boot from the persisted `<kookrDir>/boot-marker.json` marker BEFORE startup recovery runs, so it reflects the state at bind time; the graceful SIGTERM/SIGINT path flips the marker to `clean` so the next boot reads a clean restart. Deterministic, bounded, secret-free (timestamps + signal name only); kept in the last-good mirror so an offline digest can still say whether the previous exit was clean. Omitted when the classification is not wired (tests / partial harnesses). Always includes a slim `helperLlm` block (`{ paused: [{ provider, model, category, pausedUntil }], stormsSuppressed }`) from the in-memory helper-LLM auth-pause map (issue #2641): one row per paused provider, `pausedUntil` as ISO-8601, no API keys or raw error bodies. `stormsSuppressed` is the lifetime count of provider attempts refused by the process-wide attempt budget (issue #2083). When the optional systemd notifier is wired (production always wires it; issues #2491, #2853), a `systemdNotifier` block (`systemd-notifier.v1`) projects the process-local readiness/watchdog arming read from the sd_notify environment at boot: `arming` (`absent` — no `NOTIFY_SOCKET`; `notifier-only` — readiness armed, watchdog not; `watchdog-armed` — both), the `notificationEnabled` / `watchdogArmed` booleans, the `watchdogIntervalMs` heartbeat cadence (`WATCHDOG_USEC / 2`, `0` when not armed), and `externalUnitStatus` — always `"unknown"`. The block also exposes `sendHealth` for readiness and watchdog calls (issue #3227): `status` is `not-attempted`, `pending` until the first completion, then `succeeded` or `failed` from the latest helper completion. Cumulative `attempts` and `failures` saturate at `Number.MAX_SAFE_INTEGER`. `lastAttemptAt`, `lastSuccessAt`, and `lastFailureAt` are Unix-millisecond timestamps, initially null. `lastError` is null or a fixed category (`helper-missing`, `helper-exit`, `helper-signal`, `send-error`); success clears it while retaining failure history. Reads perform no `systemctl` call or filesystem work. Neither arming nor a successful helper completion proves external supervision or guarantees a restart; `kookr ops digest` warns whenever the watchdog is not armed. Always includes a four-field `timerHealth` summary (`{ registered, overdue, neverFired, oldestNeverFiredName }`) plus `oldestOverdueName` from the in-memory lifecycle-timer tracker (issue #2636): counts only — the per-loop list stays on `GET /api/diagnostics/timer-health`. `overdue` reuses the existing two-interval rule, so a loop still inside its first cadence after boot is not overdue. The last-good health mirror keeps both `helperLlm` and `timerHealth` even when the full body is truncated, so a remote operator can still name a paused helper-LLM provider and answer "did prune run?" after HTTP goes dark. `kookr ops digest` prints a warning when a provider is paused. As a side effect, each successful assembly mirrors a redacted, size-capped (≤32 KiB) copy of this body to `<kookrDir>/last-good-health.json` (atomic rotate-by-overwrite, owner-only mode `0o600`, throttled to ~5s / on a gauge edge) for the offline digest `kookr ops digest --offline` when the HTTP surface is dark (issues #2495, #2561). Always includes a `controlPlane` collection-provenance block (`control-plane-collection.v1`; issue #2798): `collectionStatus` (`ok` live+complete / `degraded` last-good or a partial round / `unavailable` nothing to serve), `source` (`live` / `last-good` / `unavailable`), `lastGoodAt` + `lastGoodAgeMs` (when the served snapshot was actually collected and how stale it is), `timedOutComponents` / `erroredComponents` (the disk-backed reads — `ciBlindDebt`, `pipelineStarvation` — that missed their per-component budget or threw this round, so one slow/failing provider degrades only itself and never zeroes the live worker/session counts), and `hookLag` (latest hook-ingestion lag + freshness from the newest processed session, or `null` when unwired). On a cold cache the first request bounds the assembly by `HEALTH_ASSEMBLY_DEADLINE_MS` (2.5s): if it does not finish in time the request serves the on-disk last-good snapshot — worker/session counts intact — marked `source: "last-good"`, or a typed `source: "unavailable"` body that omits counts (never a fabricated zero), while the single-flight assembly keeps running so the next poll recovers to live data. This is read-only observability — it never restarts an agent or the daemon. Always HTTP 200 once the listener is bound — use `/api/ready` for restart/cordon gates. Also a `projectAutomation` block (`{ pausedProjectIds, paused: [{ projectId, since? }], digest?, loadWarning? }`), distinct from `safeMode` — `safeMode.engaged=false` must not be read as “automation is running” while a project is paused. Also an optional `signalDelivery` block (issues #3046, #3225): `configured` / `consecutiveFailures` / `pending` / `scanFailures` / `scanFailedFiles` / `lastScanFailureAt` / `lastScanError` / `lastSendAt` / `lastFailureAt` / `lastError` / `nextAttemptAt` from the in-memory delivery-bridge `status()` — a cheap read (never an outbox scan) that makes a silently-failing (revoked webhook / bad token / 429) Discord/Telegram bridge glanceable, and separates per-file outbox-read failures from channel failures; on a persistent all-channel failure the bridge spaces retries with capped exponential backoff (honoring a 429 `Retry-After`) instead of re-POSTing every poll. Omitted when the bridge is unconfigured or before the service is wired at startup. Also a `processFatal` block (issue #3112): since-boot `unhandledRejectionTotal` / `uncaughtExceptionTotal` counts plus the last (capped, message-only — never the error object) `lastFatalError` and ISO-8601 `lastFatalAt`, stamped by the process fatal handlers in `start.ts`. Makes a daemon quietly absorbing fatal rejections visible here instead of only as a rotated-away `[fatal]` line in `server.log`. Cheap in-memory read; omitted when the getter is not wired (partial test harnesses). Also an `operationalAlertSink` block (issue #3315): `configured` / `writable` / optional `lastFailure.{ts,message}` from the operational-alert JSONL sink's in-memory `status()`. Distinguishes a frozen incident log (ENOSPC / EACCES) from a quiet day. Cheap in-memory read — never opens the file, never dumps alert bodies. Omitted when the sink is not wired (partial test harnesses). |
...
| `GET /api/startup-summary` | Full crash-recovery startup summary (entry lists, plus optional post-restart verification) fetched once on UI mount. Compact counts also on `GET /api/health` → `startupRecovery` (issues #2351, #3316). |
...
| `GET /api/diagnostics/session-health` | Versioned cross-signal health snapshot for tracked sessions, including signal timestamps, attach state, browser bridge state, machine-readable `unknownDetail` (reason code + next-check hint + signal ages) on `health-unknown` sessions, and coordinated-stall diagnostics |
...
| `automationEnabled` | no | boolean | Omitted or `true` = allowed. Explicit `false` pauses autonomous launches for this project and any in-memory row that shares its `localPath`. Does not flip `schedule.enabled`. |
...
| `POST /api/schedules/:id/archive` | Archive a schedule (issue #2981) — retire an abandoned loop without deleting it: the row is kept but excluded from the active fleet, so it stops firing and drops off status, health checks, and ROI attribution. Optional body `{ "reason"?: "<text>" }`. Allowed mid-run — an already-launched task still closes out its ledger row; only future firing is blocked. Returns the archived schedule. Backs `kookr schedule archive <id>`. |
| `POST /api/schedules/:id/unarchive` | Un-archive a schedule (issue #2981) — return it to the active fleet in its prior enabled/hold state. Its ROI rollup is rebuilt from the retained execution ledger, including any run that closed out after the archive. Returns the restored schedule. Backs `kookr schedule unarchive <id>`. |
...
2. Spawns at most one on-demand `repository-idea-scout` for the repo when no
   ...
   A scout that dies before a session attaches (`disposition` in
   `launch_error` / `launch_timeout` / `stale_open_launch`, typically expired
   Grok session login) does **not** write `lastStarvationScoutAt`. The next
   refill tick may retry with a salted idempotency key
   (`starvation-scout:<slug>:<bucket>:rN`), up to 3 create-then-pre-session
   deaths per 25m anti-thrash window
   (`STARVATION_SCOUT_LAUNCH_ERROR_RETRY_CAP`); further attempts audit as
   `pipeline_starvation_scout_spawn_failed` without stamping. A spawn that
   throws still does not stamp.
...
| `setProjectConfig` | Patch a tracked project's configuration. Omitted `config` keys preserve stored values; explicit `null` clears `dailyPrLimit`, `budgetWarnUsd`, `zeroDrainIssueLimit`, or `notes`. `config.automationEnabled` is a boolean (`false` pauses autonomous launches for that project). | `project`, `config` |
