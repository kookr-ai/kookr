...
| `server.pid` | server bootstrap (`src/server/single-writer-lock.ts`, RFC R27) | Single-writer lock: the owning server's pid. A planned `prod:restart` waits for this file to be absent or to name a dead/zombie pid before starting the next process — it does not signal that pid, because after a crash the number can be reused. The outgoing server closes the listen socket before it unlinks the lock (issue #2501). Incoming boot retries a few seconds, then fails loudly if another live pid still holds it. | Transient. Present while a server owns the data dir; unlinked on graceful shutdown. Safe to delete only when no Kookr process is running (next boot takes over a stale file). Do not delete while the server is up. |
