...
| `operational-alerts.jsonl` and `operational-alerts.jsonl.N` | schedule runtime / operational-alert sink | Durable trace of operational-alert fire/recovery transitions (schedule dead-man plus every resource-tick operational alert: cpu/memory/disk/RSS/circuit-breaker/persistence/provider-health), so a fire→clear that occurs while no dashboard client is connected still leaves an on-disk record (issues #1709, #1897). The active file is size-rotated via `appendJsonlWithRotation` when an append would exceed 16 MiB (2 rotated generations retained; issue #3311). Caps are code constants, not env-configurable. | Keep for incident reconstruction. |
...
| `ops-status.json` | ops-status writer (issues #1995, #2032, #3249) | Edge-triggered last-known-good ops card (ready degrade, dead-man fire, dead-man self-heal escalate-with-zero-success / later clear, pipeline starvation fire, SAFE MODE engage, prod smoke tick fire/clear): sha, hungSuspect count, data-dir free space, safeMode, recent critical edges, optional `deadManSelfHealExhausted` leftover (attempts + timestamp only — no schedule lists). Smoke fire detail is the failingChecks list only (no secrets). Written atomically; best-effort on disk-full. | Keep for post-hoc diagnosis when Discord/pages are down. |
...
| `finding-evidence-reviews.jsonl` and `finding-evidence-reviews.jsonl.N` | finding review diagnostics | Manual/background finding-evidence review outcomes. The active file is size-rotated via `appendJsonlWithRotation` when an append would exceed 8 MiB (2 rotated generations retained; issue #3047); the diagnostics endpoints and the background sampler tail only the most-recent ~2 MiB of the active file, so read cost no longer scales with total history and older rotated history is not surfaced by `readAll`. Caps are code constants, not env-configurable. | Keep if using finding review diagnostics. |
...
| `collaboration-audit.jsonl` and `collaboration-audit.jsonl.N` | collaboration/sharing | Collaboration and viewer-share audit events. The active file is size-rotated via `appendJsonlWithRotation` when an append would exceed 16 MiB (2 rotated generations retained; issue #3252). Caps are code constants, not env-configurable. Owner-only (`0o600`). | Keep for share audit history. Not touched by `kookr maintenance prune`. |
