...
| `audit.snapshot.json` | CommandJournal (`src/remote/command-journal.ts`) | Compact projection cache of the remote-command journal (idempotency map, open intents, results, grant tombstones) written when the journal compacts. Speeds restart and CLI outcome queries so the full `audit.jsonl` tail need not be replayed from zero. | Keep with `audit.jsonl`. Safe to delete while Kookr is stopped — rebuilt from the active audit on next journal open (or starts empty if no audit). Not touched by `kookr maintenance prune`. |
