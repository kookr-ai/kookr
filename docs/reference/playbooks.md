...
frontmatter becomes the agent prompt, prefixed at launch by a one-line Kookr
context header naming the playbook, its scope, and its definition file path.
See [Prompt assembly](#prompt-assembly).
...
Prompt body (the context header is prepended at launch).
```

## Prompt assembly

The prompt Kookr sends to the agent is the context header, a blank line, then
the interpolated body:

```
> _Kookr playbook context — you are executing the **My Playbook** playbook (scope: project). Its definition file is at `/path/to/.kookr/playbooks/my-playbook.md`. If asked to modify this playbook, edit that file._

<interpolated body>
```

The header exists so the agent can tell it is running a playbook at all, and so
"modify this playbook" does not require searching the playbook directories for a
file whose name the agent never saw. It is generated by `playbookContextHeader`
in [`src/server/use-cases/playbook-launch.ts`](../../src/server/use-cases/playbook-launch.ts);
it is not authorable and cannot be disabled. Body-relative file references are
normalized to absolute paths *before* the header is prepended, and parameters are
interpolated into the body only — the header is never a parameter target.

Two consequences for authors:

- Don't open the body with your own "you are running X" preamble; the header
  already says it, and two framings compete.
- For `user`- and `plugin`-scope playbooks the definition path is outside the
  agent's working directory. For `plugin` scope it points into the plugin install
  tree, where an edit is lost on the next reinstall — copy the playbook to project
  or user scope before changing it.
