...
| `KOOKR_LESSON_SPOOL` | on (unless under Vitest) | `0` to disable | Controls the lesson-write spool recovery loop (issue #1519): periodic `kb` health probes, idempotent drain of `~/.kookr/playbook-state/lesson-write-spool/`, and a 2h prolonged-degradation operational alert. Set to `0` to disable the background service (agent-side PATH shim and `kookr lesson *` CLI still work). Automatically disabled when `VITEST` is set so unit tests do not shell out to `kb doctor`. See [lesson-write-spool](lesson-write-spool.md). |
...
| `KOOKR_ALERT_PROCESS_RSS_BYTES` | `0` (disabled) | Non-negative number (bytes), `0` disables | Kookr supervisor process resident-set-size (RSS) threshold for operational alerts, evaluated like `KOOKR_ALERT_CPU_PERCENT`. Unlike host `KOOKR_ALERT_MEMORY_PERCENT`, this surfaces the Kookr process itself "fattening" (e.g. retained snapshots/hook state) before the host is near OOM. The `warning` alert carries remediation hints (inspect `/api/diagnostics/hook-ingestion` and the active task count, run `kookr maintenance prune --dry-run`, clear finished tasks). A common starting value is `2147483648` (2 GiB). |
