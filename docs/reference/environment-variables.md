...
| `KOOKR_SERVER_LOG_MAX_BYTES` | `52428800` (50 MiB) | Non-negative integer bytes | Mid-process size cap for the live `{dataDir}/server.log` (issue #1991). When the file grows past this threshold, the server renames it through numbered generations (`.1`, `.2`, …) and reopens stdout/stderr onto a fresh `server.log` so subsequent writes are not lost to the renamed inode. `0` disables mid-process rotation (restart-time rotation from `prod-restart.sh` still applies). |
