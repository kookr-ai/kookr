...
| `KOOKR_RESOURCE_WATCHDOG` | unset (disabled) | `1`/`true`/`yes`/`on` to enable | Master enable for continuous resource-watchdog sampling (issue #1724). When on, a periodic host sampler watches swap %, MemAvailable, `/proc/vmstat` `oom_kill` deltas, per-agent-family process counts, and orphan-session counts; on pressure it spawns a briefed, unattended investigation task (or a meta-reflection task after the 24h spawn budget). **Off by default** — flip on after review for continuous monitoring. Spawns go through the normal launch path (capacity/backpressure + reserved-slot posture for actor `kookr`). State: `{dataDir}/resource-watchdog.state.json`. Audit: `{dataDir}/resource-watchdog-audit.jsonl`. Health: `GET /api/health` → `resourceWatchdog` block (cached sampler state only — never a `/proc` scan on the request path). When the master is off, the same block still reports `pressureWhileDisabled` if cached `staleProcesses.dtach.count` ≥ soft bound 20 (issue #2039). With default auto-enable (see `KOOKR_RESOURCE_WATCHDOG_AUTO_ENABLE`), that soft-bound pressure also triggers a rate-limited investigation spawn without flipping this master flag (issue #2354). |
| `KOOKR_RESOURCE_WATCHDOG_AUTO_ENABLE` | `1` (on when unset) | `1`/`true`/`yes`/`on` enable; `0`/`false`/`no`/`off` page-only | When the master switch (`KOOKR_RESOURCE_WATCHDOG`) is off, still auto-enable a **rate-limited** investigation (or meta-reflection) spawn if soft-bound host pressure is already tripping (`staleProcesses.dtach.count` ≥ soft bound; issue #2354). Uses the same throttle + 24h spawn budget as the continuous actuator. Audits `auto_enable` then `trigger`/`spawn`. Set to `0` for deliberate page-only mode (Discord alerter still fires; no spawn). Does not restart prod or kill processes. |
