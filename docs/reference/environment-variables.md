...
| `KOOKR_RETRO_VERIFY_QUEUE_DIR` | `~/.kookr/playbook-state/retro-verify-queue` | Absolute or relative path | Override the durable retro-verify queue directory (issue #1689) — merges made while CI was signal-absent are enqueued here (SHA + PR + reason) and re-verified when capacity recovers. |
