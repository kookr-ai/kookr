...
| `KOOKR_READY_URL` | `http://127.0.0.1:${KOOKR_PORT}/api/ready` | HTTP URL | Readiness endpoint used by `scripts/prod-restart.sh` as the deploy gate (issue #1721). Returns 503 with `checks.startup.reason=startup-in-progress` until post-listen recovery finishes; 200 when ready for work. |
