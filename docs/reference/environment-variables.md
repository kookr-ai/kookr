...
| `KOOKR_ADMIN_TOKEN` | unset | Secret string | Optional admin token accepted via `x-kookr-admin-token` for `/api/admin/*` routes from non-loopback callers. Loopback admin requests are trusted without this token. |
