...
| `KOOKR_SERVER_LOG_MAX_BYTES` | `52428800` (50 MiB) | Non-negative integer bytes | Mid-process size cap for the live `{dataDir}/server.log` (issue #1991). When the file grows past this threshold **and** process stdout is attached to that path (script redirect `> server.log`), the server renames it through numbered generations (`.1`, `.2`, …) and reopens stdout/stderr onto a fresh `server.log` so subsequent writes are not lost to the renamed inode. Skips freopen when stdio is a TTY or journald (does not steal those streams). `0` disables mid-process rotation (restart-time rotation from `prod-restart.sh` still applies). |
| `KOOKR_LOG_GENERATIONS` | `3` | Non-negative integer, max 100 | How many rotated `server.log.N` generations to retain. Shared by mid-process rotation (issue #1991) and `scripts/prod-restart.sh` restart-time rotation. `0` disables rotation on both paths. Aged generations are also eligible for `kookr maintenance prune`. |
| `KOOKR_SERVER_LOG_ROTATE_INTERVAL_MS` | `60000` (60s) | Non-negative integer ms | How often the server stats `server.log` for the size-cap check. `0` disables the timer (restart-time rotation still applies). |
...
| `KOOKR_RELAY_ORPHAN_SWEEP_INTERVAL_HOURS` | `1` | Non-negative number of hours (`0` disables) | Cadence of the server-side relay-orphan sweep (issue #1723 / #1885): reaps leaked `relay/server.ts` processes that either (a) have a deleted task worktree, or (b) carry a test-runner env fingerprint (KOOKR_RELAY_DIE_WITH_PARENT or any VITEST* marker) — a stranded test/e2e relay whose worktree is still present — the always-on backstop for the die-with-parent watchdog. **On by default (1h)** as of #1885 — the previous off-by-default meant nothing actively reaped in production and orphans re-accumulated to ~29 procs / ~1.5 GB after #1723; set `0` to disable. A one-shot reclaim also runs ~30s after boot so a long-lived daemon sheds already-accumulated orphans without a restart. Production-safe — a live relay's working directory always exists and it never carries the test marker, so it is never selected; each reap logs pid/age/RSS, and a failing sweep is logged without crashing the server. |
| `KOOKR_HOST_STALE_DTACH_REAP` | on | `0`/`false`/`off` disables | Master enable for the host-stale dtach reaper (issue #2356). When on, a periodic sweep reclaims process-table `kookr-dtach` masters that are **not** live-attached, whose socket file is gone, and that are aged past the teardown-race floor — **only when** host-wide `staleProcesses.dtach.count` ≥ soft bound. Distinct from the session reaper (#1720), which only reaps backend-reported live sessions. Fail-closed pure policy; kill path is `killProcessTree` (TERM→grace→KILL) on selected pids only. |
| `KOOKR_HOST_STALE_DTACH_REAP_INTERVAL_MINUTES` | `5` | Non-negative minutes (`0` disables timer) | Cadence of the host-stale dtach reaper. A one-shot reclaim also runs ~45s after boot. |
| `KOOKR_HOST_STALE_DTACH_REAP_DRY_RUN` | off | `1`/`true`/`yes`/`on` | Log would-reap pids without signalling. Useful to validate selection before enabling kills. |
| `KOOKR_HOST_STALE_DTACH_REAP_SOFT_BOUND` | `20` | Non-negative integer (`0` disables gate) | Pressure gate: no kills when `staleProcesses.dtach.count` is below this (same default as `resourceWatchdog.pressureWhileDisabled`). |
| `KOOKR_HOST_STALE_DTACH_REAP_MAX_PER_SWEEP` | `5` | Non-negative integer | Rate limit: max masters reaped (or dry-run logged) per sweep. |
| `KOOKR_HOST_STALE_DTACH_REAP_MIN_AGE_MS` | `60000` | Non-negative ms | Minimum process age before a missing-socket master may be selected (teardown-race floor). |
| `KOOKR_RELAY_ORPHAN_ALERT_BOUND` | `5` | Non-negative integer | Bound for the `relayOrphanFinding` first-class health finding (issue #1885). When `/api/health` `staleProcesses.relayServer.count` strictly exceeds this, the health payload gains a `relayOrphanFinding` block (`{ code: "relay_orphan_accumulation", count, bound, rssBytes }`) so sentinel/reflection can cite a stable code instead of re-deriving a threshold from raw counts. Visibility only — it never reaps or flips readiness. |
| `KOOKR_REFLECT_WORKTREE_SWEEP_INTERVAL_HOURS` | `1` | Non-negative number of hours (`0` disables) | Cadence of the server-side reflect-worktree orphan sweep (issue #1860): reaps ephemeral directories under `<dataDir>/reflect-worktrees` whose source task no longer has a live reflect task (plus a 7-day mtime TTL backstop that can reclaim even linked dirs). Startup still sweeps once at boot; this timer is the periodic backstop so long-lived instances do not accumulate dirs after reflect-task crashes. Default `1` hour when unset; set to `0` to disable. Removed/kept counts are logged each tick, and a failing sweep is logged without crashing the server. |
| `KOOKR_PROD_SMOKE_TICK` | on for port 4800, off otherwise | `1`/`true` to enable, `0`/`false`/`off`/`no` to disable | Controls the hourly in-process prod smoke tick (issue #1593): runs the same bounded checks as the post-deploy smoke suite (`/api/ready` + `/api/health` response bounds, `/api/tasks?limit=1` latency, adapter version-probe sanity) against the live instance on a schedule, and files/updates a single operational alert artifact (`{dataDir}/prod-smoke-tick-alert.json`) on failure so a wedge that develops while the server runs is caught within an hour instead of sitting undetected. Enabled by default only on the canonical prod port (`4800`) so a fresh deploy is protected with no operational change; dev servers and the unit-test suite stay silent (the suite sets this to `0` via `vitest.config.ts`). No agent is spawned for the happy path. Health: `GET /api/health` → `prodSmokeTick` block (issue #2031) projects `status`, `consecutiveFailures`, and `failingChecks` from the alert artifact only — never re-runs smoke checks on the request path; the block is omitted when this flag disables the tick. |
...
| `KOOKR_DEPLOY_CONVERGENCE` | on for port 4800, off otherwise | `1`/`true` to enable, `0`/`false`/`off`/`no` to disable | In-process deploy-convergence controller (issue #2226): every few minutes asserts the serving SHA includes `origin/main`, and past the 15m grace POSTs `/api/deploy/trigger`. Also pages residual operator signal `deploy:stale-residual` when `behindCount≥1` and `deploying=false` for the residual stale window (default 20m). Independent of the agent schedule. Hold file `~/.kookr/deploy-convergence-hold` suppresses auto-redeploy (residual still pages). |
| `KOOKR_DEPLOY_CONVERGENCE_INTERVAL_MINUTES` | `5` | Positive minutes | Cadence of the in-process controller. Non-positive disables it. |
| `KOOKR_DEPLOY_CONVERGENCE_ACT` | `true` | `1`/`true` or `0`/`false` | When false, classify + residual only — never call `/api/deploy/trigger`. |
| `KOOKR_DEPLOY_CONVERGENCE_GRACE_MINUTES` | `15` | Non-negative minutes | Override the DIVERGENT grace window (default from `DEFAULT_CONVERGENCE_THRESHOLDS`). |
| `KOOKR_DEPLOY_STALE_RESIDUAL_MINUTES` | `20` | Positive minutes | How long `behindCount≥1` + `deploying=false` may persist before the residual operator page. |
...
| `KOOKR_LESSON_DECISION_GATE` | on | `0` / `false` / `off` / `no` to disable | When on (default), `POST /api/tasks/:id/signal` with `kind=completion_ready` is rejected (`409 lesson_decision_required`, or `409 lesson_decision_hooks_missing` when every session hook log is gone) unless the task's hook trail shows a lesson write (`kb remember` / `kookr lesson remember`) or an explicit `No generic KB lesson:` skip. Fail-open for tasks with zero sessions. See [lesson-decision-gate](lesson-decision-gate.md). |
| `KOOKR_MERGE_REQUIRED_GATE` | on | `0` / `false` / `off` / `no` to disable | When on (default), `POST /api/tasks/:id/signal` with `kind=completion_ready` is rejected (`409 merge_required`) for merge-authority tasks that opened a PR whose merge is unverified and lack a `PR-BLOCKER:` trail marker. Merge authority is opt-in (TERMINAL-STATE CONTRACT `mergeAfterImplementation=true`, playbook param, or explicit stamp). Live check prefers `gh pr view --json mergedAt`. Fail-open when hooks dir is unset or the task has no merge authority. See [merge-required-gate](merge-required-gate.md). |
| `KOOKR_MERGE_REQUIRED_LIVE_VERIFY` | on (off under Vitest) | `0` / `false` / `off` / `no` to force trail-only; `1` / `true` / `on` to force live | Controls live `gh pr view --json mergedAt` probes used by the merge-required gate. Default on outside test runners; auto-off when `VITEST` is set so hermetic suites do not shell out. |
...
| `KOOKR_REAP_ORPHAN_AGE_MS` | `86400000` (24h) | Non-negative integer ms | Minimum age an UNOWNED session must reach before the reaper above terminates it in steady state — avoids racing a mid-launch session (#1537 item 2). |
| `KOOKR_REAP_ORPHAN_AGE_UNDER_PRESSURE_MS` | `7200000` (2h) | Non-negative integer ms | Under-pressure unowned-orphan age (issue #2081). When the dtach pressure soft bound is met (preferred signal: host-wide `staleProcesses.dtach.count` ≥ 20 via a 15s-cached /proc scan, same bound as `pressureWhileDisabled`; falls back to this instance's live session count when /proc is unavailable), the session reaper uses this shorter threshold instead of `KOOKR_REAP_ORPHAN_AGE_MS` so unowned masters reclaim before a full-day wait risks OOM. Alias: `KOOKR_SESSION_REAP_ORPHAN_AGE_UNDER_PRESSURE_MS` (issue wording; the preferred `KOOKR_REAP_ORPHAN_AGE_UNDER_PRESSURE_MS` name wins if both are set). Effective age is logged on every sweep and reported on `GET /api/health` → `sessionReaper.effectiveOrphanAgeMs`. |
| `KOOKR_RESOURCE_WATCHDOG` | unset (disabled) | `1`/`true`/`yes`/`on` to enable | Master enable for the resource watchdog actuator (issue #1724). When on, a periodic host sampler watches swap %, MemAvailable, `/proc/vmstat` `oom_kill` deltas, per-agent-family process counts, and orphan-session counts; on pressure it spawns a briefed, unattended investigation task (or a meta-reflection task after the 24h spawn budget). **Off by default** — a new actuator; flip on after review. Spawns go through the normal launch path (capacity/backpressure + reserved-slot posture for actor `kookr`). State: `{dataDir}/resource-watchdog.state.json`. Audit: `{dataDir}/resource-watchdog-audit.jsonl`. Health: `GET /api/health` → `resourceWatchdog` block (cached sampler state only — never a `/proc` scan on the request path). When disabled, the same block still reports `pressureWhileDisabled` if cached `staleProcesses.dtach.count` ≥ soft bound 20 (issue #2039) so operators see host pressure while auto-investigation is off — visibility only; does not flip this flag on. |
...
| `KOOKR_TERMINAL_ATTACH_VIEWPORT_BYTES` | `65536` (64 KiB) | Positive integer bytes | Default streaming attach seed size for viewport-first attach (issue #1934). Cold attaches send the last N bytes of the ring (with mid-CSI / UTF-8 safe cut rules) instead of the full 1 MiB ring. |
| `KOOKR_TERMINAL_ATTACH_FULL_RING` | unset | `1`/`true`/`yes`/`on` to enable | Rollback flag (issue #1934): force full-ring streaming attach instead of viewport-first seed. Absolute-TUI path is unaffected (never full-ring smash). |
| `KOOKR_TERMINAL_ATTACH_ALWAYS_HISTORY` | unset | `1`/`true`/`yes`/`on` to enable | Alias of `KOOKR_TERMINAL_ATTACH_FULL_RING` (issue #1934) for operators who want streaming attaches to always load full history. |
...
| `KOOKR_SESSION_BRIDGE_INITIAL_RESIZE_WAIT_MS` | `50` | Non-negative integer milliseconds (`0` disables the wait) | How long an owner terminal bridge waits for the browser's first FitAddon `resize` control frame before deciding whether to replay the ring buffer. Kept short because TerminalPanel sends resize in `onOpen` and absolute-TUI seed frames can paint before size arrives. Raise only if FitAddon is systematically late (slow layout). |
...
| `KOOKR_LLM_AUTH_COOLDOWN_MS` | `1800000` (30 minutes) | Non-negative integer milliseconds (`0` disables) | After a helper-LLM provider fails with category `auth` (invalid/expired API key, 401/403), `FallbackLlmClient` skips that provider/model until this cool-down elapses instead of re-hitting dead credentials on every call. Paused providers appear on the helper-LLM diagnostics snapshot (`pausedProviders`). Invalid/negative values fall back to the default. |
| `KOOKR_LLM_PROVIDER_ATTEMPT_BUDGET` | `90` | Non-negative integer (`0` disables) | Process-wide max helper-LLM *provider network attempts* allowed inside the sliding window (issue #2083). Counts every fallback-chain hop across all `FallbackLlmClient` instances — including successful calls — so free-tier 429 cascades cannot thrash the event loop under multi-task load. When exhausted, further hops degrade without network and `helperLlm.stormsSuppressed` increments. Auth-paused providers do not burn this budget. Invalid/negative values fall back to the default. |
| `KOOKR_LLM_PROVIDER_ATTEMPT_WINDOW_MS` | `60000` (60 seconds) | Positive integer milliseconds | Sliding-window size for `KOOKR_LLM_PROVIDER_ATTEMPT_BUDGET`. Non-positive or non-numeric values fall back to the default. Live usage appears on helper-LLM diagnostics as `providerAttemptBudget`. |
...
| `KOOKR_ALERT_PROVIDER_FALLBACK_SUBSTITUTIONS` | `5` | Non-negative number (count), `0` disables | Provider-health durable-alert threshold (issue #1897, #1699 WS1.5). Fires one standing `warning` alert once at least this many provider fallback substitutions land within `KOOKR_ALERT_PROVIDER_FALLBACK_WINDOW_MS`, so failover masking a chronic provider outage becomes an explicit operational alert (also written to the durable `operational-alerts.jsonl` sink). Clears with one `info` alert once substitutions fall out of the window and the pool resumes. Requires the window to be `> 0`; set to `0` to disable the substitution rule. Inert until the dispatch/failover substitution counter feeds real signals. |
| `KOOKR_ALERT_PROVIDER_FALLBACK_WINDOW_MS` | `600000` (10 min) | Non-negative number (milliseconds), `0` disables | Rolling window over which `KOOKR_ALERT_PROVIDER_FALLBACK_SUBSTITUTIONS` fallback substitutions are counted for the provider-health alert. Setting either this or the substitution count to `0` disables the substitution rule (the paused-duration rule still applies). |
| `KOOKR_ALERT_PROVIDER_PAUSED_MS` | `300000` (5 min) | Non-negative number (milliseconds), `0` disables | Provider-pool paused-duration threshold for the provider-health alert (issue #1897). Fires the same standing `warning` alert when the provider pool has been continuously paused for at least this long, independent of the substitution rule, and clears once the pool resumes. Set to `0` to disable the paused-duration rule. |
| `KOOKR_PROVIDER_PAUSED_HARD_TTL_MS` | `7200000` (2 h) | Positive number (milliseconds), clamped to a 6 h maximum | Hard TTL for continuous `provider_paused` task holds (issue #2079). After this long of effective pause age (first-observed pause start, advanced by later liveness so sticky billing events alone do not kill a recovering agent), the liveness tick terminates the task with disposition `provider_paused_ttl` / needs-human — never auto-completed as delivered. Open-PR fail-safe and awaiting-provider-reset (#1896) still skip reclaim. Invalid / non-positive / unset uses the default. Health: `GET /api/health` → `providerPausedOccupancy`. |
| `KOOKR_PROVIDER_PAUSED_SOFT_TTL_MS` | `2400000` (40 m) | Positive number (milliseconds), always clamped at or below the hard TTL (and never below 15 m) | Soft TTL for capacity-aware early reclaim of `provider_paused` holds (issue #2225). When free general-source slots ≥ 3 and phantom+paused occupancy ≥ 4, the liveness tick uses this bound instead of waiting for the hard 2 h TTL, so aged pauses free concurrency while product work could still launch. Open-PR fail-safe and awaiting-provider-reset still skip. Invalid / non-positive / unset uses the default. Health: `providerPausedOccupancy.softTtlMs` / `effectiveTtlMs` / `capacityEarlyReclaim`. |
| `KOOKR_FINISHED_AWAITING_ACK_SOFT_TTL_MS` | `300000` (5 m) | Positive number (milliseconds), always clamped at or below the hard finishedAwaitingAck TTL (and never below 2 m) | Soft TTL for capacity-pressure early reclaim of `awaiting_poll` finishedAwaitingAck phantoms (issue #2355). When the pending queue is empty and either effective utilization is below 75% or `phantomActive ≥ 4`, the liveness tick may force-complete `awaiting_poll` FAA at this bound instead of waiting for the hard 15 m TTL — converting phantom slots into free capacity. Never soft-reclaims `manual_review_gate` / `auto_close_disabled` / `ack_sweep_backlog`; open-PR fail-safe still blocks. Invalid / non-positive / unset uses the default. Health: `finishedAwaitingAckTtlReclaim.softTtlMs` / `capacityEarlyReclaim` / `capacityPressureEarlyReclaimedTotal` / lastOutcomes `capacity_pressure_early_reclaim`. |
...
| `KOOKR_ADMISSION_RETRY_AFTER_SECONDS` | `2` | Integer `>= 1` (seconds) | `Retry-After` hint sent on the load-based 503s above (event-loop saturation #1590 and data-directory disk-critical #1992) and reused as the default drain/redeploy Retry-After on launch 503s (issue #1976). Unset/blank uses the default; a **non-integer or `< 1`** value is rejected by startup config-preflight (fatal). |
| `KOOKR_ADMISSION_DATA_DIR_FREE_PERCENT` | same as `KOOKR_ALERT_DATA_DIR_FREE_PERCENT` (`5`) | Non-negative number (percent free), `0` disables this floor | Optional override for the data-directory free-percent floor used by **fail-closed launch admission** (issue #1992). When unset, reuses `KOOKR_ALERT_DATA_DIR_FREE_PERCENT` so ops alerts and admission share one knob. Free space at or below this floor for `KOOKR_ADMISSION_DATA_DIR_SUSTAIN_SAMPLES` consecutive resource ticks sheds `POST /api/tasks` with **HTTP 503** + `code`/`reason` `data_directory_disk_critical` before any task is created (reclaim/reap paths stay open). Both this floor and the bytes floor at `0` disables disk admission. |
| `KOOKR_ADMISSION_DATA_DIR_FREE_BYTES` | same as `KOOKR_ALERT_DATA_DIR_FREE_BYTES` (`2147483648`) | Non-negative number (bytes free), `0` disables this floor | Optional override for the data-directory free-bytes floor used by fail-closed launch admission (issue #1992). When unset, reuses `KOOKR_ALERT_DATA_DIR_FREE_BYTES`. OR'd with the percent floor — either breach counts. |
| `KOOKR_ADMISSION_DATA_DIR_SUSTAIN_SAMPLES` | same as `KOOKR_ALERT_SUSTAIN_SAMPLES` (`3`) | Integer `>= 1` | Optional override for consecutive breaching resource-status ticks required before disk-critical admission rejects launches (issue #1992). When unset, reuses `KOOKR_ALERT_SUSTAIN_SAMPLES` so a single noisy sample cannot false-positive the gate shut. Samples are taken roughly every 2 seconds. |
...
| `KOOKR_SNAPSHOT_SHED_EVENT_LOOP_DELAY_MS` | `1500` | Non-negative number (milliseconds), `0` disables | Server-side full-snapshot-rebuild shed threshold (issue #1818). Reuses the SAME sampled server event-loop delay p95 as `KOOKR_ADMISSION_EVENT_LOOP_DELAY_MS` / `KOOKR_WS_LOAD_SHED_EVENT_LOOP_DELAY_MS` (no second `monitorEventLoopDelay`). When the latest sample is **strictly greater** than this value, a non-critical full snapshot *rebuild* is skipped (fail-open) so the saturated loop can serve terminal I/O; `0` disables shedding entirely. Invalid/blank or non-finite values use the default. |
| `KOOKR_LOOP_DELIVERY_NO_PROGRESS_SAMPLES` | `3` | Integer `>= 0`, `0` disables | Delivery-aware loop watchdog threshold (issue #1902, WS2.4 of #1699). A self-sustaining Ralph loop is sampled once per iteration with its cumulative delivery counters — branch commit count (which advances as the loop pushes commits to its open PR), PRs opened, and PRs merged; when none of them advance for this many CONSECUTIVE iterations the loop is flagged as delivered-then-hung (looks alive, delivers nothing) with a single `[loop-delivery-watchdog]` warn line. It never inspects token/hook/pane activity, so silence alone can never flag a loop and a quiet-but-progressing loop is never flagged. `0` disables the watchdog entirely. Invalid/blank values use the default. |
| `KOOKR_LOOP_DELIVERY_RECOVER_SAMPLES` | `2` | Integer `>= 1` | Consecutive delivery-progress iterations required to CLEAR a flagged loop (issue #1902). Asymmetric with `KOOKR_LOOP_DELIVERY_NO_PROGRESS_SAMPLES` by design so a loop oscillating around the boundary can't flap the flag every iteration. Invalid/blank values use the default. |
| `KOOKR_LOOP_TERMINAL_RELAUNCH` | `true` (enabled) | `false` to disable | Terminal-loop relaunch policy (issue #1901, WS2.3 of #1699). When a Ralph loop hits a relaunch-eligible terminal exit (`iteration_cap` / `target_stalled` / `all_targets_stalled`), the loop is re-armed in place with a fresh iteration budget — gated by the WS0.5 relaunch arbiter (issue #1711) so concurrent actuators cannot double-launch the same issue and a post-run cooldown throttles thundering-herd relaunch. Set to `false` to disable automatic relaunch (the loop just stops as before). Independent of this flag, a **budget-exhausted** terminal exit (`cost_cap` / `iteration_cost_cap`) always escalates the loop to a visible needs-human state instead of relaunching. |
| `KOOKR_LOOP_TERMINAL_RELAUNCH_MAX` | `20` | Integer `>= 0`, `0` = unbounded | Cap on cumulative automatic terminal relaunches (issue #1901) before a loop escalates to a visible needs-human state (`relaunch_exhausted`) instead of re-arming again. Bounds runaway relaunch of a structurally-stuck loop that has no cost cap to backstop it. The count is preserved across re-arms (not reset with the iteration counter). `0` disables the cap for loops that legitimately relaunch indefinitely. Invalid/blank/negative values use the default. |
...
| `KOOKR_WS_DELTA` | `true` (enabled) | `1`/`true`/`on`/`yes` or `0`/`false`/`off`/`no` | Stage-2 delta WebSocket protocol (issue #1754). When enabled (default), after the first full-snapshot baseline of a process the dashboard hot path fans out a coalesced `delta` frame (changed agents / removals / relations / aggregates) instead of re-serializing the full ~MB fleet on every change. Connect, `requestResync`, and soft-backpressure re-base still send full `snapshot` frames. Set to a falsy value to force full-snapshot fan-out (instant kill-switch). Requires the stream sequencer (`serverEpoch`); invalid/blank values use the default. |
...
| `KOOKR_PRS_PER_DAY` | unset | Non-negative number (PRs/day), invalid/blank ignored | Operator/supervisor-supplied recent PR-per-day throughput fed to the supply-aware capacity signal (issue #2143) on `GET /api/health` → `idleCapacityFinding`. Judged against `KOOKR_TARGET_PRS_PER_DAY` only to populate the finding's `atOrAboveThroughputTarget` field — utilization is deliberately NOT an escalation key, so throughput below target never by itself raises the finding to `warning`. Absent ⇒ the finding reports `prsPerDay: null` and stays pure idle-slot observability. |
| `KOOKR_TARGET_PRS_PER_DAY` | unset | Non-negative number (PRs/day), invalid/blank ignored | Configured PR-per-day target the supply-aware capacity signal judges `KOOKR_PRS_PER_DAY` against (issue #2143). Used for the reported `atOrAboveThroughputTarget` flag only; never gates severity on its own. Absent ⇒ `targetPrsPerDay: null`. |
| `KOOKR_VETTED_IDEA_BACKLOG` | unset | Non-negative number (count), invalid/blank ignored | Vetted-idea backlog depth — the numerator of the vetted-idea runway metric (issue #2143). Combined with `KOOKR_VETTED_IDEA_CONSUMPTION_PER_DAY` it yields `vettedIdeaRunwayDays` (backlog ÷ consumption), surfaced on `GET /api/health` → `vettedIdeaRunway` and `idleCapacityFinding`. Runway below `KOOKR_VETTED_IDEA_RUNWAY_FLOOR_DAYS` is the actual scarce-resource signal that escalates the idle-capacity finding to `warning` (`driver: runway_shortfall`). Requires BOTH backlog and consumption to be set; absent ⇒ no runway is computed. |
| `KOOKR_VETTED_IDEA_CONSUMPTION_PER_DAY` | unset | Non-negative number (ideas/day), invalid/blank ignored | Recent vetted-idea consumption rate — the denominator of the runway metric (issue #2143). A non-positive rate leaves runway unbounded (`runwayDays: null`, never a shortfall). Requires `KOOKR_VETTED_IDEA_BACKLOG` to also be set to compute a runway. |
| `KOOKR_VETTED_IDEA_RUNWAY_FLOOR_DAYS` | `2` | Non-negative number (days), invalid/blank uses default | Runway floor for the vetted-idea supply signal (issue #2143). When known runway falls below this many days, the idle-capacity finding escalates to `warning` and the supervisor should trigger idea-supply work instead of chasing utilization. |
| `KOOKR_QUOTA_FETCH_TIMEOUT_MS` | `30000` | Positive integer milliseconds | Per-request deadline for the Anthropic OAuth quota-usage poller's `fetch` calls (issue #2159). Bounds each request with an `AbortController` so a hung usage endpoint aborts the poll — routed into the adapter's normal failure/backoff path — instead of silently stalling the poll loop and freezing the `quotaStatus` broadcast. Invalid/blank or non-positive values fall back to the default. |
...
| `KOOKR_CONTRIBUTION_LEDGER_MAX_BYTES` | `4194304` (4 MiB) | Positive integer bytes | Size cap for the active `contribution-ledger.jsonl` file written by `OssAttemptStore.appendLedgerEntry`. When an append would push the file past this cap, the store rotates it to `contribution-ledger.jsonl.1` (shifting older generations up) via the shared `appendJsonlWithRotation` helper (issue #2331). Shell contribution hooks still append with `>>` and do not size-rotate on write. Non-positive or unparseable values fall back to the default. |
| `KOOKR_CONTRIBUTION_LEDGER_ROTATE_KEEP` | `2` | Positive integer | Number of rotated contribution-ledger generations (`.1` … `.N`) retained before the oldest is deleted, bounding on-disk ledger history to roughly `(keep + 1) × KOOKR_CONTRIBUTION_LEDGER_MAX_BYTES`. `OssAttemptStore.loadFromLedger` and the shell OSS contribution gate both read the active file plus retained generations so daily/weekly rate-limit math stays correct within the retention window after rotation. Non-positive or unparseable values fall back to the default. |
