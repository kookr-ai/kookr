...
| Overdue or never-fired hourly timer | `timerHealth.overdue` | `timerHealth.overdue >= 1`, or an hourly loop has `lastFiredAt=null` and server uptime already exceeds its interval (issue #2637). Prefer `GET /api/health.timerHealth` when it includes `loops[]` or an explicit `neverFired` count. A missing/`null` block, or a bare `{ overdue: 0 }` that cannot answer never-fired, triggers a 2-second fetch of [`GET /api/diagnostics/timer-health`](./api.md) |
