...
| Overdue or never-fired hourly timer | `timerHealth.overdue` | `timerHealth.overdue >= 1`, or an hourly loop has `lastFiredAt=null` and server uptime already exceeds its interval (issue #2637). Prefer `GET /api/health.timerHealth`; if that block is missing or `null`, digest does a 2-second fetch of [`GET /api/diagnostics/timer-health`](./api.md) |
