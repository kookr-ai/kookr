...
- `archive <id>` (issue #2981) retires an abandoned loop **without deleting it**: the schedule is kept but excluded from the active fleet, so it stops firing and drops off `kookr status`, the health checks, and ROI attribution — unlike a plain `disable`, which leaves a schedule "active" and still costing health checks. Add `--reason <text>` to record why. `unarchive <id>` returns it to the active fleet in its prior enabled/hold state. Use `list --archived` to see what has been archived.
