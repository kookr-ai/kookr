...
/** Runtime qualification derived SOLELY from the manifest (no duplicate allowlist). */
export function qualifyBuild(
  m: CompatibilityManifest,
): 'tested' | 'unknown' | 'known-incompatible' {
  // A build is only `tested` when it was reviewed AND every hard gate passes.
  if (m.reviewStatus !== 'tested') return m.reviewStatus;
  const hardGatesOk = m.gates.every((g) => g.verdict === 'pass' || g.verdict === 'warn');
