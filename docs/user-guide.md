...
Before you confirm, the dialog names the worktree and reports whether it can actually be removed, using the same inspection the cleanup itself runs — so a checked box means it really will go, and a disabled one means it really won't. A worktree that is dirty, unmerged, shared with another active task, protected, or still driven by a running Ralph loop is reported as kept, and the checkbox is disabled with the reason. Expand **why?** for the evidence (changed-file counts, commits ahead of the default branch, and the full path). Because the verdict is a snapshot, use the re-check control (**↻**) after committing or merging elsewhere to refresh it without closing the dialog; it is hidden for reasons that can never change, such as the repository's primary checkout.
