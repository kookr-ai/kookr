...
To repeat a prompt you have run before — without writing a playbook — open the Launch dialog's **Manual** tab and expand **Recent prompts**. It lists your recent manual-launch prompts, most recent first, with prompts you previously launched against the current working directory floated to the top so "the prompt I use for repo X" is one click away. Type in the filter box to narrow the list. Selecting an entry fills the task description with that exact text (it does not submit, and does not change the working directory, agent, or completion criteria — adjust those yourself, then Launch). Recall reads from the server (the live task list plus the durable archive), so it works on a fresh browser and includes prompts sent from other sessions; it is not affected by clearing your browser data. It shows nothing when you have no recent manual launches, and is not offered when you are relaunching an existing task (that flow already prefills from the original task).
