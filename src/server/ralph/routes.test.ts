...
describe('Ralph route ownership', () => {
  test('top-level route composer registers Ralph routes outside task routes', () => {
    ...
    const ralphRoutesSource = readFileSync(new URL('./routes.ts', import.meta.url), 'utf8');
    ...
    expect(taskRoutesSource).not.toContain("app.post('/api/playbooks/ralph-loop'");
    expect(taskRoutesSource).not.toContain("app.post('/api/tasks/:taskId/ralph-loop/replace-with-new'");
    expect(taskRoutesSource).not.toContain('canonicalizeTarget');
    expect(ralphRoutesSource).toContain('type RalphRouteDeps = Pick<');
    expect(ralphRoutesSource).toContain("app.post('/api/playbooks/ralph-loop'");
    expect(ralphRoutesSource).toContain("app.post('/api/tasks/:taskId/ralph-loop/replace-with-new'");
  });
});
...
`);
  ...
  test('accepts split playbook source and task target without legacy cwd', async () => {
    ...
    const res = await mkApp(mkLoopDeps(taskStore)).request('/api/playbooks/ralph-loop', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
...
`);
  ...
  test('keeps playbook replace available by default and matches keys against target cwd', async () => {
    ...
    const mismatch = await mkApp(mkLoopDeps(taskStore)).request(
    ...
    const success = await mkApp(mkLoopDeps(taskStore)).request(
      `/api/tasks/${old.id}/ralph-loop/replace-with-new`,
      {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
