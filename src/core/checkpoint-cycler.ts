...
const PROMPT_MESSAGE = (ratioPercent: number): string =>
  `Context window is at ${ratioPercent}% of the model limit. Before I run /compact, please update $TASK_CHECKPOINT_DIR/CHECKPOINT.md with the current verdict, evidence, and next actions, and update $TASK_CHECKPOINT_DIR/CHECKPOINT.json using schema_version "semantic-checkpoint.v1" with task_id, repo, worktree, branch, verdict, decisions, evidence, files_changed, tests_run, open_risks, next_actions, and memory_write_candidates. If review-only memory updates exist, write or preserve $TASK_CHECKPOINT_DIR/${MEMORY_WRITE_CANDIDATES_FILENAME} using schema_version "${MEMORY_WRITE_CANDIDATES_SCHEMA_VERSION}" with target, evidence, verifier.status, approval.status, lifecycle, and promotion metadata; do not promote candidates automatically. After your reply I will run /compact for you, then a fresh turn will read CHECKPOINT.json first when valid and fall back to CHECKPOINT.md automatically.`;
