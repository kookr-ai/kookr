...
import { evaluateAttestation, evaluateStructural, isSourceFile, parseChecklist } from './checklist.js';
...
export function verifyChecklist(input: VerifyInput): VerifyReport {
  ...
  const results = [...attest.results, ...structural];
...
export async function collectAndVerify(deps: CollectDeps): Promise<VerifyReport> {
  ...
  return verifyChecklist({ body: deps.body, facts, envFileText, testCorpus });
}
