...
describe('GrokBuildAdapter', () => {
  ...
  test('forwards independent model and effort overrides to the harness', async () => {
    const adapter = makeAdapter();
    const task = taskStore.createTask('pinned grok', '/workspace');
    const sessionId = await adapter.launch(task.id, 'pinned grok', '/workspace', undefined, {
      model: 'grok-4.6',
      effort: 'high',
    });
    expect(backend.sessions.get(sessionId)!.spec.args).toEqual([
  ...
  describe('injectHookEvent', () => {
    ...
    test('dispatches bypass permission_denied as tool_error, not permission_request', async () => {
      const { adapter, sessionId } = await launched();
      const events: AgentEvent[] = [];
      adapter.onEvent((_id, e) => events.push(e));
      adapter.injectHookEvent(sessionId, JSON.stringify({ hookEventName: 'session_start', sessionId: 'S1', cwd: '/workspace' }));
      ...
      expect(events.find((e) => e.type === 'tool_error')).toMatchObject({
        type: 'tool_error',
        toolName: 'run_terminal_command',
