...
**Kookr ownership claim** (RFC issue-ownership-lock PR 1b) — the issue-claim
**lease** is this pipeline's single machine dedup source (#1711/#1904),
complementary to the GitHub claim comment. It is the only machine lock: there is
no separate delivery/file lock. Because the claim is durable (rebuilt from tasks
on restart), it prevents a restarted run or a concurrent pipeline from
double-submitting the same issue. Claim it before Phase 4's fix/push:
...
# exit 6 → lease held by another live task → re-scout a different candidate (R16);
#          after pass+retry POST /api/issue-claims/exhausted
