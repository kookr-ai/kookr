...
- **Drain cap (issue #1657).** `kookr emission plan` additionally caps the budget by the **target repo's own drain rate** — the count of issues it *closed* in the last 7 days. A repo draining ~1 issue/window admits ~0–1 new issues/window even when its absolute backlog sits under 60. This is keyed on `--repo` (the repository being filed into), never the emitting actor's home repo, so a high-drain actor filing into a low-drain repo is throttled by the low-drain target. If the drain count cannot be read, the plan refuses to emit rather than falling back to an uncoupled budget.
- **Zero-drain allowance (issue #2804).** A target repo with no drained issues inherits an unlimited allowance (`-1`) unless the installation sets `KOOKR_MAX_ZERO_DRAIN_ISSUE_LIMIT`, in which case it inherits that deployment ceiling. The repository owner may set `zeroDrainIssueLimit` beside the other project settings in Kookr; set it explicitly to `0` to refuse zero-drain emission. A project-specific value cannot exceed the optional deployment ceiling.
