...
- **Zero-drain allowance (issue #2804).** A target repo with no drained issues remains refused by default. The repository owner may set `zeroDrainIssueLimit` beside the other project settings in Kookr; `0` or an omitted value keeps the refusal. The optional deployment variable `KOOKR_MAX_ZERO_DRAIN_ISSUE_LIMIT` can cap what any repository on that Kookr installation may configure, without imposing a built-in product limit.
