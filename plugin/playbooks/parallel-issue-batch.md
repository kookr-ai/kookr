...
- Before an autonomous self-merge, run the `independent-merge-review` skill: spawn a **fresh-context** reviewer (blind to your implementation reasoning) that reviews the diff and posts a machine-readable verdict PR comment with an exact `review-head-sha`. Codex is the primary reviewer lane; if Codex is unavailable or rate-limited, the Claude fallback lane runs. **Repo-policy split (issue #3027) — pick one lane, do not mix them.** On `kookr-ai/kookr` (hard gate): never degrade to zero review; fix or explicitly rebut each confirmed correctness/safety finding before merging; `pnpm merge` enforces this deterministically (it refuses to merge without a `pass` verdict bound to the current head; the `review-skipped-timeout` label is telemetry only); retry up to the durable default of 10 correction/review iterations, then record a concrete blocker; do not set `KOOKR_MERGE_REQUIRE_REVIEW=0` for an autonomous merge. On a repo whose `CLAUDE.md` / `AGENTS.md` documents that independent review is advisory (e.g. jeanibarz/lucy#3606): still post the verdict when the reviewer returns; timeout, missing, or BLOCK never become a task blocker or merge refusal; merge after local gates (`gh pr merge`; `KOOKR_MERGE_REQUIRE_REVIEW=0 gh pr merge` is authorized on that advisory autonomous merge if a global hook still denies the bare command). Classify: `kookr-ai/kookr` → hard; else `grep -qiE 'independent review is advisory' CLAUDE.md AGENTS.md` → advisory; else hard.
