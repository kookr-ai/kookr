...
- Before an autonomous self-merge, run the `independent-merge-review` skill: spawn a **fresh-context** reviewer (blind to your implementation reasoning) that reviews the diff and posts a machine-readable verdict PR comment with an exact `review-head-sha`. Codex is the primary reviewer lane; if Codex is unavailable or rate-limited, the Claude fallback lane runs — never degrade to zero review. Fix or explicitly rebut each confirmed correctness/safety finding before merging. On `kookr-ai/kookr`, `pnpm merge` enforces this deterministically (it refuses to merge without a `pass` verdict bound to the current head; the `review-skipped-timeout` label is telemetry only). Retry up to the durable default of 10 correction/review iterations, then record a concrete blocker. Do not set `KOOKR_MERGE_REQUIRE_REVIEW=0` for an autonomous merge.
