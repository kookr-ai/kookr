...
**Autonomous loops and merge safety:** `automation-design` (design simple feedback loops and refine them from failures and verified outcomes across projects), `autonomous-review-loop` (bounded implement-and-correct cycles with durable accounting), `autonomous-watch-loop` (long-lived poll-and-act janitor tasks), `self-continuation-task` (sequential task chains where each run spawns the next), `independent-merge-review` (fresh-context exact-head verdict: hard merge gate on kookr-ai/kookr; advisory evidence where the repo documents independent review as advisory — issue #3027).
