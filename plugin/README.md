...
**OSS contribution:** `oss-fork-manager`, `oss-pr-{critic,distill,plan,state,threshold}`, `oss-repo-recon`, `pr-review-triage`, `pr-contribution-excellence` (bundled with per-repo distilled patterns under `repo/`), `find-best-reviewers`, `rust-lang-rust-contributions`.
