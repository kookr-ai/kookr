...
export function createRelayModeTest(mode: 'stored' | 'runtime') {
  ...
  return base.extend<{ suppressOnboarding: boolean }, { serverURL: string }>({
    ...
    serverURL: [async ({}, use) => {
      ...
      const port = await new Promise<number>((resolve, reject) => {
        ...
        const timeout = setTimeout(() => {
          proc.kill();
          reject(new Error(
            `${label} relay test server did not start within 15 s.\nstdout: ${stdout}\nstderr: ${stderr}`,
          ));
        }, 15_000);
        ...
        proc.on('exit', (code) => {
          if (!stdout.includes('E2E_PORT=')) {
            clearTimeout(timeout);
            reject(new Error(
              `${label} relay test server exited (code ${code}) before ready.\nstdout: ${stdout}\nstderr: ${stderr}`,
            ));
          }
        });
      });
