...
//
// Scoped strictly to relays this suite spawned — matched by the
// KOOKR_RELAY_DIE_WITH_PARENT marker in their environment — so a developer's
// separately-running local/prod relay on the same machine is never touched.
//
...
import { classifyProcess, isTestSpawnedRelayEnviron } from '../src/core/orphan-process-scanner.js';
...
export function findLingeringTestRelays(
  ...
  for (const proc of listProcesses()) {
    ...
    if (!isTestSpawnedRelayEnviron(readEnviron(proc.pid))) continue;
    pids.push(proc.pid);
  }
  return pids;
}
