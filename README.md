...
Prerequisites: `git`, Node.js `>=22`, `pnpm >=10`, and build tools for native modules. Claude Code is only required when you want Kookr to launch Claude Code agents; install it with `curl -fsSL https://claude.ai/install.sh | bash` and see [Claude Code Setup](https://code.claude.com/docs/en/setup).
